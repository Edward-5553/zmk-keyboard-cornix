...
所有层旋钮功能一致：左旋钮顺时针向下滚动、逆时针向上滚动；右旋钮顺时针增大音量、逆时针减小音量。左旋钮按下切换 Caps Lock，右旋钮按下切换静音（键位 30/31）。基础层原 Caps 拇指键为空，Fn/Symbol 层同位置仍为 F1。左侧使用鼠标滚轮事件，滚动量受系统设置影响。首次启用鼠标功能后，若蓝牙连接不能滚动，需刷新主机的 HID 缓存（通常移除设备后重新配对）。
